...
> **文档类型：Canonical 设计**
> **事实基线：2026-08-16**
> **篇章边界：本篇用于跨实现选型、稳定输出与接入治理；自研 `QueryImpl` 细节由 [6.3](./03-QueryAndIteration.md) 维护。**
...
三种实现是并列可选能力，而不是同一 ECS 的三级后端。框架只统一 World/Host 的组合位置，不统一组件 schema、查询语义和结构修改协议；项目应按实体规模、第三方生态、数据布局和同步要求选择其一或明确划分各自所有的数据域。
...
| 实现 | E0-E2 | E3-E5 | 当前可用结论 |
|------|-------|-------|--------------|
| 轻量 ECS | 源码可审计；Release 构建 0 警告、0 错误 | 无独立 Query 契约测试和专项 gate | 适合轻量、单线程、可读性优先的逻辑；热点与结构修改需自行测量 |
| Entitas | 适配源码与 MOBA 消费；Release 构建成功但有 8 个兼容警告 | 无适配层独立 E3/E5 | 适合项目愿意采用 generated contexts、Group/Reactive 生态的场景 |
| Svelto | 适配源码与 Shooter 消费；Release 构建 0 警告、0 错误 | Shooter 仅提供局部消费者 E3；无适配层专项 E5 | 适合明确 group/数据布局和 submission 所有权的高密度模拟 |

当前没有覆盖三种实现相同业务语义的统一兼容测试，因此不能宣称它们可以透明互换。确定性输出应在项目协议层按稳定业务键排序并建立 golden/hash 测试，而不是依赖任一底层集合的原始遍历顺序。

### 7.2 选择检查表

1. 先确定权威实体规模、查询组合和结构变化频率，再选择数据模型。
2. 明确谁创建、Tick、提交和 Dispose ECS 资源，不让业务系统隐式接管 world 所有权。
3. 对快照、hash、回放和网络输出建立显式稳定排序。
4. 用目标场景测量分配和耗时，不从 struct API、Group 缓存或 Native 容器推断零分配。
5. 多 ECS 共存时按数据域隔离，不让同一权威组件在两个存储中形成无 owner 的双写状态。
