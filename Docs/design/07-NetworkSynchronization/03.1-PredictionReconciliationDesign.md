...
> 帧同步整世界重演、状态同步权威覆盖后局部重演，以及每类对象的预测资格见 [帧同步与状态同步预测回滚方案](03.2-FrameStatePredictionRollback.md)。
...
| MOBA Entitas 与 Context 增量 | **已实现**：新增 ActorId/SkillCastInstanceId 实体集合、Context EntityId 游标和部分生命周期/技能组件快照，预测新实体清除时裁剪召唤索引。 | 确认实体已销毁时预校验拒绝恢复；Motion、Buff、Flow 与外部引用未形成任意实体自动重建。只证明局部逻辑恢复，不证明 View/Cue 已重整。 |
...
*文档版本：v3.3 | 最后更新：2026-09-16 | 文档类型：Canonical 设计 | 状态：表现对账桥仍由项目实现并验收*
