---
name: cs-issue
description: 诊断或修复 bug、报错、性能回退或既有行为异常。不用于新功能（openspec-propose）或行为等价重构（cs-refactor）。
argument-hint: "[问题描述]"
---
...
- 诉求其实是新增能力而不是坏掉的行为时，转 `openspec-propose`，不在 issue 里偷做。
...
- 修复中发现根因超出已授权 scope（例如必须重构或新增功能）时，先报告影响与最小选项并请 owner
  决定；不得借修复擅自扩大改动。
