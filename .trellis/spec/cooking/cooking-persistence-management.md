...
> 交付状态：**completed-limited-scope**；原完整能力迁移状态为 `blocked`。本规范由只读来源快照 `.trellis/migration/legacy-cooking-changes/add-cooking-persistence-management/specs/cooking-persistence-management/spec.md` 转换；原始 SHA-256 见 [迁移清单](../../migration/legacy-cooking-changes/manifest.json)。
>
> 本文件保留行为规则与历史场景；只有顶部列明的 in-memory pure .NET limited delivery 已验证。durable/product 后续见 successor backlog，Unity 见 prohibited future scope。
