...
> 本文件保留遗留设计及已验证的受限实施记录；2026-09-16 最终范围按 in-memory pure .NET limited delivery 收口，不表示 durable storage 或完整 P5 完成。
...
本轮实际覆盖 P01–P09 的 in-memory technical subset 与 P10 `BlockedByOwnerDecision`。存档 owner、save timing、exit/power loss/cancel/host exit、migration/backup/quarantine/recovery、encryption/key policy 与真实 durable store 均未选择或实现，只进入 successor backlog；Unity-host integration 只见 prohibited future scope。它们不是本 archived limited delivery 的 blocker。
...
原规划曾假定应用代码放入 Unity cooking package；该指令现为只读历史且不可执行。已交付代码保持在纯 .NET `src/AbilityKit.Game.Cooking*`；任何 Unity-host 工作只见 prohibited future scope，non-Unity durable/app-host 后续只见 successor backlog。
