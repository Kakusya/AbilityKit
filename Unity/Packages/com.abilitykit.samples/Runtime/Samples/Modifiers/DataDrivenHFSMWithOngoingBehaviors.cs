    ...
    public sealed class DataDrivenHFSMWithOngoingBehaviors : SampleBase
    {
        ...
        private bool LoadFSMConfig()
        {
            try
            {
                ...
                Log($"  找到配置: {configPath}");
                var json = ResourceProviders.Current.LoadText(configPath);

                // Newtonsoft matches camelCase JSON to PascalCase members by default.
                _fsmConfig = JsonConvert.DeserializeObject<HFSMConfig>(json);
                return true;
            }
            catch (Exception ex)
            {
        ...
        private bool LoadOngoingConfig()
        {
            try
            {
                ...
                // Newtonsoft matches camelCase JSON to PascalCase members by default.
                _ongoingConfig = JsonConvert.DeserializeObject<OngoingBehaviorSetConfig>(json);
                return true;
            }
            catch (Exception ex)
            {
        ...
        private string FindConfigFile(string relativePath)
        {
            ...
            foreach (var path in searchPaths)
            {
                ...
                if (ResourceProviders.Current.Exists(fullPath))
                {
                    return fullPath;
                }
            }
