# com.abilitykit.samples

> AbilityKit 的示例体系，用来帮助新人从零到一理解框架理念、模块边界和按需组合方式。

## 包结构

| 路径 | 内容 |
| --- | --- |
| `Runtime/Abstractions/` | 宿主边界契约：`ISample` / `SampleBase` / `SampleCatalog` / `IResourceProvider` / `ILogger` / `ISampleEnvironment` |
| `Runtime/Samples/` | 分级示例本体（18 个分类） |
| `Runtime/Infrastructure/` | 配置加载、注册表与示例基础设施 |
| `Runtime/Ability/` | Legacy：一套教学用的迷你骨架，仅保留参考，不进 manifest 与学习路径（见 `Document/SamplesLogicRefactorPlan.md` §6.3） |
| `Runtime/sample-manifest.json` | 稳定示例入口：id、order、title、status、level、guide、learningContract、next 学习链 |
| `Document/` | 示例路线图、覆盖计划与重构计划 |

`Runtime` 的 asmdef 声明 `noEngineReferences: true`——示例内容是纯逻辑，在编译期就禁止触碰 `UnityEngine`，把"逻辑与表现分离"变成硬约束。

## 示例定位

本包不绑定 Unity、MonoGame 或具体服务器框架。它只保留玩法逻辑、配置模型、上下文、日志和时间推进等纯 C# 内容，运行环境由外层宿主提供。

这意味着同一份 sample 可以：

- 在控制台运行并输出日志。
- 写入文件，方便回归检查和教学留档。
- 由 Unity 宿主驱动（逐步接入中），或接入 MonoGame、服务器宿主，由宿主提供时间、资源、配置和输出。
